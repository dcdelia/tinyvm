...
void Parser::handleHelpCommand() {
    ...
    std::cerr << std::endl << "Demo OSR points can be inserted with one of the following commands:" << std::endl
              ...
              << "\tCOND is ALWAYS, NEVER, or COUNT <THRESHOLD>" << std::endl
    ...
    std::cerr << std::endl << "The command can either update F1 or generate a new function F1' cloning F1 "
              ...
              << std::endl;
    std::cerr << std::endl << "At the moment, counter-based OSR conditions are supported in UPDATE mode only."
              << std::endl;
...
void Parser::openOSRHelper(Function* src, Instruction* OSRSrc, bool update,
        ...
        int threshold, bool dynInline, Value* valToDynInline) {
    ...
    if (threshold) {
        assert(update && "OSR counter supported in UPDATE mode only!");

        // insert alloca now that live variables have already been computed
        AllocaInst* alloca = TheHelper->insertOSRCounter(src);
        TheHelper->generateCounterTriggeredCond(&cond, alloca, threshold);
    }
...
void Parser::resolvedOSRHelper(Function* src, Instruction* OSRSrc, bool update,
        ...
        F2NewName, OSRLibrary::OSRCond &cond, int branchTakenProb, int threshold) {
    ...
    if (threshold) {
        ...
        // inserting an alloca would not affect the set of live variables to
        // fetch, as it depends on the destination function only
        AllocaInst* alloca = TheHelper->insertOSRCounter(src);
        TheHelper->generateCounterTriggeredCond(&cond, alloca, threshold);
    }
...
void Parser::handleInsertOSRCommand() {
    ...
    if (!strcasecmp(token, "ALWAYS")) {
        ...
        getToken();
        threshold = (int)strtol(token, NULL, 10);
        if (threshold < 1) INVALID();
    } else {
        INVALID();
    }
    ...
    if (threshold && !update) {
        std::cerr << "Sorry, for the time being OSR points triggered by a"
                  << "counter are supported in UPDATE mode only!" << std::endl;
        return;
    }
    ...
    if (open) {
        ...
        openOSRHelper(src, OSRSrc, update, tmpForF1NewName, cond, branchTakenProb,
                threshold, dynInline, valToInline);
        delete cmdLine;
        return;
    }
    ...
    resolvedOSRHelper(src, OSRSrc, update, tmpForF1NewName, &P2Name,
            const_cast<std::string*>(&F2NewName), cond, branchTakenProb, threshold);

/*
 * OSR-enabled version of lib/Transform/Utils/LoopSimplify.cpp (LLVM 3.6.2)
 */
...
static void placeSplitBlockCarefully(BasicBlock *NewBB,
  ...
  NewBB->moveAfter(FoundBB); /* OSR: nothing to do (affects code layout only) */
}
...
///
BasicBlock* OSR_InsertPreheaderForLoop(Loop *L, Pass *PP, CodeMapper* OSR_CM) {
  ...
  if (!Header->isLandingPad()) {
    PreheaderBB = OSR_SplitBlockPredecessors(Header, OutsideBlocks, ".preheader",
                                             PP, OSR_CM);
  } else {
    SmallVector<BasicBlock*, 2> NewBBs;
    OSR_SplitLandingPadPredecessors(Header, OutsideBlocks, ".preheader",
                                ".split-lp", PP, NewBBs, OSR_CM);
...
static BasicBlock *rewriteLoopExitBlock(Loop *L, BasicBlock *Exit, Pass *PP,
  ...
  if (Exit->isLandingPad()) {
    SmallVector<BasicBlock*, 2> NewBBs;
    OSR_SplitLandingPadPredecessors(Exit, LoopBlocks,
...
static Loop *separateNestedLoop(Loop *L, BasicBlock *Preheader,
                                ...
                                AssumptionCache *AC, CodeMapper* OSR_CM) {
  ...
  PHINode *PN = findPHIToPartitionLoops(L, AA, DT, AC, OSR_CM);
  ...
  BasicBlock *Header = L->getHeader();
  BasicBlock *NewBB =
    OSR_SplitBlockPredecessors(Header, OuterLoopPreds,  ".outer", PP, OSR_CM);
  ...
  L->moveToHeader(Header); /* OSR: nothing to do (it affects code layout only) */
...
static BasicBlock *insertUniqueBackedgeBlock(Loop *L, BasicBlock *Preheader,
  ...
  for (BasicBlock::iterator I = Header->begin(); isa<PHINode>(I); ++I) {
    ...
    if (OSR_CM) OSR_CM->addInstruction(NewPN); /* OSR */
    if (AA) AA->copyValue(PN, NewPN); /* OSR: TODO set alias info */
...
ReprocessLoop:
  ...
  for (Loop::block_iterator BB = L->block_begin(), E = L->block_end();
    ...
    for (BasicBlock *P : BadPreds) {
      ...
      if (OSR_CM) {
          OSR_CM->addInstruction(tmpUnreachable);
      }
      Changed = true;
    }
  }
  ...
  if (!Preheader) {
    Preheader = OSR_InsertPreheaderForLoop(L, PP, OSR_CM);
    if (Preheader) {
      ++NumInserted;
      Changed = true;
    }
  ...
  for (SmallSetVector<BasicBlock *, 8>::iterator I = ExitBlockSet.begin(),
    ...
    for (pred_iterator PI = pred_begin(ExitBlock), PE = pred_end(ExitBlock);
      ...
      // allowed.
      if (!L->contains(*PI)) {
        if (rewriteLoopExitBlock(L, ExitBlock, PP, OSR_CM)) {
          ++NumInserted;
          Changed = true;
        }
        break;
  ...
  if (!LoopLatch) {
    ...
    if (L->getNumBackEdges() < 8) {
      if (Loop *OuterL =
              separateNestedLoop(L, Preheader, AA, DT, LI, SE, PP, AC, OSR_CM)) {
        ++NumNested;
    ...
    // loop header.
    LoopLatch = insertUniqueBackedgeBlock(L, Preheader, AA, DT, LI, OSR_CM);
    if (LoopLatch) {
      ++NumInserted;
      Changed = true;
    }
  ...
  if (UniqueExit) {
    for (unsigned i = 0, e = ExitingBlocks.size(); i != e; ++i) {
      ...
      for (BasicBlock::iterator I = ExitingBlock->begin(); &*I != BI; ) {
        ...
        if (Inst == CI)
          continue;
        if (!OSR_makeLoopInvariant(Inst, AnyInvariant,
      ...
      // to fold it now.
      /* [OSR] if (!OSR_FoldBranchToCommonDest(BI, DL)) continue; */
      if (!OSR_FoldBranchToCommonDest(BI, DL, 1, OSR_CM)) continue;
...
bool OSR_simplifyLoop(Loop *L, DominatorTree *DT, LoopInfo *LI, Pass *PP,
...
namespace {
  struct OSR_LoopSimplify : public FunctionPass {
    static char ID; // Pass identification, replacement for typeid
    CodeMapper* OSR_CM;
    OSR_LoopSimplify() : FunctionPass(ID) {
      initializeOSR_LoopSimplifyPass(*PassRegistry::getPassRegistry());
    }
...
char OSR_LoopSimplify::ID = 0;
OSR_INITIALIZE_PASS_BEGIN(OSR_LoopSimplify, "OSR_loop-simplify",
                "[OSR] Canonicalize natural loops", false, false)
OSR_INITIALIZE_PASS_DEPENDENCY(AssumptionCacheTracker)
OSR_INITIALIZE_PASS_DEPENDENCY(DominatorTreeWrapperPass)
OSR_INITIALIZE_PASS_DEPENDENCY(LoopInfo)
OSR_INITIALIZE_PASS_END(OSR_LoopSimplify, "OSR_loop-simplify",
                "[OSR] Canonicalize natural loops", false, false)

// Publicly exposed interface to pass...
char &OSR_LoopSimplifyID = OSR_LoopSimplify::ID; /* OSR: declared as extern in OptPasses.hpp */
Pass* OSR_createLoopSimplifyPass() { return new OSR_LoopSimplify(); }
...
bool OSR_LoopSimplify::runOnFunction(Function &F) {
  ...
  for (LoopInfo::iterator I = LI->begin(), E = LI->end(); I != E; ++I)
    Changed |= OSR_simplifyLoop(*I, DT, LI, this, AA, SE, DL, AC, OSR_CM);
...
void OSR_LoopSimplify::verifyAnalysis() const {

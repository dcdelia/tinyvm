/*
 * OSR-enabled version of lib/Transform/Scalar/SCCP.cpp (LLVM 3.6.2)
 */
...
//#include <llvm/Transforms/IPO.h> /* OSR: no ISCCP at the moment */
...
namespace {
  ...
  ///
  struct OSR_SCCP : public FunctionPass {
    void getAnalysisUsage(AnalysisUsage &AU) const override {
      AU.addRequired<TargetLibraryInfo>();
    }
    static char ID; // Pass identification, replacement for typeid
    CodeMapper* OSR_CM; /* OSR */
    OSR_SCCP() : FunctionPass(ID) {
      initializeOSR_SCCPPass(*PassRegistry::getPassRegistry());
    }
...
static void DeleteInstructionInBlock(BasicBlock *BB, CodeMapper* OSR_CM) { /* OSR */
  ...
  while (EndInst != BB->begin()) {
    ...
    Instruction *Inst = --I;
    if (!Inst->use_empty()) {
      /* [OSR] Inst->replaceAllUsesWith(UndefValue::get(Inst->getType())); */
      Constant* tmpUndef =  UndefValue::get(Inst->getType());
      if (OSR_CM) OSR_CM->replaceAllUsesWith(Inst, tmpUndef);
      Inst->replaceAllUsesWith(tmpUndef);
    }
...
//
bool OSR_SCCP::runOnFunction(Function &F) {
  ...
  for (Function::iterator BB = F.begin(), E = F.end(); BB != E; ++BB) {
    if (!Solver.isBlockExecutable(BB)) {
      /* [OSR] DeleteInstructionInBlock(BB); */
      DeleteInstructionInBlock(BB, OSR_CM);
      MadeChanges = true;
      continue;
    }

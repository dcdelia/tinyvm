/*
 * OSR-enabled version of lib/Transform/Scalar/EarlyCSE.cpp (LLVM 3.6.2)
 */
...
/// cases.
class OSR_EarlyCSE : public FunctionPass {
public:
  ...
  static char ID;
  explicit OSR_EarlyCSE() : FunctionPass(ID) {
    initializeOSR_EarlyCSEPass(*PassRegistry::getPassRegistry());
  }
...
char OSR_EarlyCSE::ID = 0;

// createEarlyCSEPass - The public interface to this file.
FunctionPass* OSR_createEarlyCSEPass() {
  return new OSR_EarlyCSE();
}

OSR_INITIALIZE_PASS_BEGIN(OSR_EarlyCSE, "OSR_early-cse", "[OSR] Early CSE", false, false)
...
OSR_INITIALIZE_PASS_END(OSR_EarlyCSE, "OSR_early-cse", "[OSR] Early CSE", false, false)

bool OSR_EarlyCSE::processNode(DomTreeNode *Node) {
